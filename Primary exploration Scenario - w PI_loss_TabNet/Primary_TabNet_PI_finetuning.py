...
RESULT_DIR = "alpha_sweep_results_tabnet"
OUT_DIR    = "finetune_results_tabnet"
...
class MonotonicTabNet(Model):
    def __init__(self, input_dim, cell_col_idx, margin=0.0, alpha=1.0,
                 n_steps=3, n_d=64, relaxation_factor=1.5):
        super().__init__()
...
model = MonotonicTabNet(input_dim=len(feat_cols), cell_col_idx=cell_idx,
...
plt.savefig(os.path.join(
    OUT_DIR,
    f"ft_curve_MAE_alpha{best_alpha}_support{SUPPORT_N}_epoch{best_epoch}.pdf"
))
plt.show()
...
plt.savefig(os.path.join(
    OUT_DIR,
    f"ft_curve_MSE_alpha{best_alpha}_support{SUPPORT_N}_epoch{best_epoch}.pdf"
))
plt.show()

model_best = MonotonicTabNet(input_dim=len(feat_cols), cell_col_idx=cell_idx,
                             margin=MARGIN, alpha=best_alpha)
