# finetune_from_pretrained_mlp_multi_cellwise.py
...
weights_path = os.path.join(RESULT_DIR, "mlp_baseline_pretrained.h5")
...
for c in [5,7]:
    ...
    # Model init & load pretrained
    model = MonotonicMLPTanh(input_dim=len(feat_cols), cell_col_idx=cell_idx, margin=MARGIN)
    ...
    model_best = MonotonicMLPTanh(input_dim=len(feat_cols), cell_col_idx=cell_idx, margin=MARGIN)
