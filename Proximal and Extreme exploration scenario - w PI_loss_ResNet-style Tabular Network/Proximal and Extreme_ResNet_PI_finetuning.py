# finetune_resnet_baseline_multi_PI_full.py
...
OUT_DIR    = "finetune_results_resnet_PI"
...
class MonotonicResNetTanh(Model): 
    ...
    def _mono_violation_loss(self, x):
        with tf.GradientTape() as g:
            g.watch(x); y_ = self.call(x, training=True)
        J = g.batch_jacobian(y_, x); grads = tf.squeeze(J, axis=1)
...
for c in [5,7]:
    ...
    model = MonotonicResNetTanh(input_dim=len(feat_cols), cell_col_idx=cell_idx,
    ...
    model_best = MonotonicResNetTanh(input_dim=len(feat_cols), cell_col_idx=cell_idx,
                                     margin=MARGIN, alpha=best_alpha)
