...
RESULT_DIR = "tabnet_baseline_results"  
OUT_DIR    = "finetune_tabnet_results" 
...
class TabNetBaseline(Model):
    def __init__(self, input_dim, n_steps=3, n_d=64, n_a=64, relaxation_factor=1.5):
        super().__init__()
        self.n_steps = n_steps
        self.relaxation_factor = relaxation_factor

        self.shared = [Dense(n_d, activation="tanh") for _ in range(2)]
        self.steps_ft = [[Dense(n_d, activation="tanh") for _ in range(2)] for _ in range(n_steps)]
        self.att_transformers = [Dense(input_dim, activation="softmax") for _ in range(n_steps)]
        self.out = Dense(1)
    ...
    def call(self, x, training=False):
        ...
        for step in range(self.n_steps):
            mask = self.att_transformers[step](masked_x) * priors
            mask = mask / (tf.reduce_sum(mask, axis=1, keepdims=True) + 1e-8)
            x_masked = x * mask
            z = x_masked
            for layer in self.shared:
                z = layer(z)
            for layer in self.steps_ft[step]:
                z = layer(z)
            out_agg += self.out(z)
            priors = priors * (self.relaxation_factor - mask)
            masked_x = z
        return out_agg
...
weights_path = os.path.join(RESULT_DIR, "tabnet_baseline_pretrained.h5")
...
for c in [5,7]:
    ...
    model = TabNetBaseline(input_dim=len(feat_cols))
    ...
    model_best = TabNetBaseline(input_dim=len(feat_cols))
    ...
    top20, top10, top5 = sorted_idx[int(n*0.8):], sorted_idx[int(n*0.9):], sorted_idx[int(n*0.95):]
    ...
    df_extra.to_excel(extra_xlsx); print(f" Saved additional R² results to {extra_xlsx}")
