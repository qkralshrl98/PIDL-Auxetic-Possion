...
weights_path = os.path.join(PRETRAINED_DIR, "ftt_pretrained.weights.h5")  
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, f"ft_curve_MAE_support{SUPPORT_N}_epoch{best_epoch}.pdf"))
plt.show()
...
plt.tick_params(axis="both", which="both", direction="in")
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, f"ft_curve_MSE_support{SUPPORT_N}_epoch{best_epoch}.pdf"))
plt.show()
