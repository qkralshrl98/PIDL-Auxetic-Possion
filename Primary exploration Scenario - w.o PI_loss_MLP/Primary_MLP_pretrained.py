...
RESULT_DIR = "mlp_baseline_results"
...
model = PlainMLP(input_dim=X_train.shape[1])
...
ckpt = ModelCheckpoint(
    filepath=os.path.join(RESULT_DIR, "mlp_baseline_pretrained.h5"),
    monitor="val_mae",
    save_best_only=True,
...
model.load_weights(os.path.join(RESULT_DIR, "mlp_baseline_pretrained.h5"))
...
mask_02 = y_val_true_inv.ravel() < 0.2
if mask_02.sum() > 0:
    results["R2_true<0.2"] = r2_score(y_val_true_inv[mask_02], y_val_pred_inv[mask_02])

mask_01 = y_val_true_inv.ravel() < 0.1
if mask_01.sum() > 0:
    results["R2_true<0.1"] = r2_score(y_val_true_inv[mask_01], y_val_pred_inv[mask_01])

mask_005 = y_val_true_inv.ravel() < 0.05
if mask_005.sum() > 0:
    results["R2_true<0.05"] = r2_score(y_val_true_inv[mask_005], y_val_pred_inv[mask_005])

sorted_idx = np.argsort(y_val_true_inv.ravel())
n = len(sorted_idx)
...
results["R2_top20%"] = r2_score(y_val_true_inv[top20], y_val_pred_inv[top20])
results["R2_top10%"] = r2_score(y_val_true_inv[top10], y_val_pred_inv[top10])
results["R2_top5%"]  = r2_score(y_val_true_inv[top5],  y_val_pred_inv[top5])

print("\n Additional R² Results:")
for k,v in results.items():
    print(f"{k}: {v:.4f}")

df_results = pd.DataFrame(list(results.items()), columns=["Condition", "R2"])
excel_path = os.path.join(RESULT_DIR, "additional_R2_results.xlsx")
df_results.to_excel(excel_path, index=False)
print(f"\n Additional R² results saved to {excel_path}")
