...
model = PlainMLP(input_dim=len(feat_cols))
...
ckpt = ModelCheckpoint(
    os.path.join(OUT_DIR, "best_epoch.weights.h5"),
    monitor="val_mae",
    save_best_only=True,
...
model_best = PlainMLP(input_dim=len(feat_cols))
_ = model_best(tf.zeros((1, len(feat_cols)), dtype=tf.float32))
model_best.load_weights(os.path.join(OUT_DIR, "best_epoch.weights.h5"))
