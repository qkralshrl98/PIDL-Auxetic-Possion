...
from tensorflow.keras.layers import Dense, LayerNormalization, MultiHeadAttention, Embedding  # Dropout 제거
...
OUT_DIR    = "finetune_baseline_results"   
...
weights_path = os.path.join(RESULT_DIR, "ftt_pretrained.h5")

def run_finetune_for_target(target_cell: int):
    print(f"\n================  Fine-tune for target cell {target_cell}  ================")
    ...
    df_tgt = df[df["cell"] == target_cell].copy()
    support_df = df_tgt.sample(n=SUPPORT_N, random_state=SEED)
    query_df   = df_tgt.drop(support_df.index)
    ...
    print(f" Best epoch by val_mae (cell {target_cell}): {best_epoch}")
    ...
    plt.savefig(os.path.join(
        OUT_DIR,
        f"ft_curve_MAE_cell{target_cell}_support{SUPPORT_N}_epoch{best_epoch}.pdf"
    ))
    plt.show()
    ...
    plt.savefig(os.path.join(
        OUT_DIR,
        f"ft_curve_MSE_cell{target_cell}_support{SUPPORT_N}_epoch{best_epoch}.pdf"
    ))
    plt.show()

    model_best = FTTransformer(
        num_num=len(num_cols),
        num_cat=len(cat_cols),
        categories_cardinalities=cat_card,
        d_model=256, num_heads=4, num_layers=3, ff_dim=128, dropout=0.1
    ...
    print(f"\n Support Set (Best Epoch, cell {target_cell})")
    ...
    print(f"\n Query Set (Best Epoch, cell {target_cell})")
    ...
    out_xlsx = os.path.join(OUT_DIR, f"query_best_epoch_results_cell{target_cell}_support{SUPPORT_N}.xlsx")
    ...
    print(f"\n Additional R² Results (Query Set, Best Epoch, cell {target_cell}):")
    for k,v in extra_results.items():
        print(f"{k}: {v:.4f}")

    df_extra = pd.DataFrame([extra_results]); df_extra.index = ["R2"]
    extra_xlsx = os.path.join(OUT_DIR, f"query_additional_R2_cell{target_cell}_support{SUPPORT_N}.xlsx")
    df_extra.to_excel(extra_xlsx)
    print(f" Saved additional R² results to {extra_xlsx}")

for tgt in [5, 7]:
    run_finetune_for_target(tgt)
